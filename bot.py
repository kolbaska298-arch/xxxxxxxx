...
async def start_handler(message: Message) -> None:
    if is_group(message):
        await message.answer(
            "Анти-Рейд активен. Контролирую флуд опросами, стикерами и медиафайлами. "
...
async def content_handler(message: Message, bot: Bot) -> None:
    if message.from_user is None or await is_moderator_or_creator(bot, message):
        return
    ...
    if message.sticker is not None:
        ...
        if len(events) > STICKER_LIMIT:
            events.clear()
            await mute_user(bot, message, 30, "саси бибу by DmTeam")
        return
    ...
    if is_media:
        ...
        if len(events) > MEDIA_LIMIT:
            events.clear()
            await mute_user(bot, message, 15, "превысил лимит медиафайлов")
